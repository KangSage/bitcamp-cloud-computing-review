...
if (id == undefined) { // 입력 폼.
    ...
    $.getJSON(ideaServerApiAddr + `json/member/view/${id}`,
        function (result) {
            data = result;
            if (data.member == null) {
                alert('아이디가 무효합니다.');
...
$(eUpdateBtn).click(function() {
    $.post(ideaServerApiAddr + '/json/member/update',
        {
            id: $(eId).val(),
            email: $(eEmail).val(),
...
$(eDeleteBtn).click(function() {
    $.getJSON(ideaServerApiAddr + `/json/member/delete?id=${eId.value}`,
        function (data) {
            if (data.status == 'success') {
                location.href = `list.html?page=${page}&size=${size}`;
            } else {
...
$(eAddBtn).click(function() {
    $.post(ideaServerApiAddr + '/json/member/add',
        {
            id: $(eId).val(),
            email: $(eEmail).val(),
            password: $(ePassword).val()
